...
const API_BOLETO =
  'https://api.unimedpatos.sgusuite.com.br/api/procedure/p_prcssa_dados/p_0177_json_busca_boleto';
...
const printMetrics: PrintMetrics = {
  total: 0,
  ok: 0,
  error: 0,
};

async function buscarUrlBoleto(numeroFatura: string | number): Promise<string | undefined> {
  const token = await getToken();
  const data = await requireOkJson<BoletoResponse>(API_BOLETO, {
    method: 'POST',
    headers: {
...
async function sendToLprPrinter(buffer: Buffer, fileName: string) {
  ...
  const writeAndWaitAck = (socket: net.Socket, data: Buffer | string) =>
    new Promise<void>((resolve, reject) => {
      socket.write(data, (err) => {
        if (err) return reject(err);
        socket.once('data', (ack) => {
          if (ack[0] === 0) return resolve();
...
async function sendToPrintService(payload: Record<string, any>) {
...
export const boletoRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/api/boleto/print', async (request, reply) => {
    ...
    const body = request.body as { numeroFatura?: string | number; url?: string };
    const { numeroFatura } = body ?? {};
    let { url } = body ?? {};
    ...
    try {
      ...
      return reply.code(500).send({ error: (error as Error)?.message || 'Falha ao imprimir' });
    }
  });
